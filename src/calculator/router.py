...
from src.calculator.schemas import ZakatOnProperty, \
    ZakatOnLivestock, ZakatUshrResponse, ZakatUshrRequest, ZakatUshrItem, NisabValueResponse, NisabValueRequest, \
    ZakatOnPropertyCalculated, \
...
@router.get("/nisab-value", response_model=NisabValueResponse)
async def get_nisab_value(request: NisabValueRequest):
    silver_price = await fetch_silver_value(request.currency)
    nisab_value = NisabValueResponse(nisab_value=int(silver_price * 612.35), currency='RUB')
    return nisab_value
